...
void LCD_ALL_INIT(void)
{
 ...
 /*=====================3-固定文字显示======================*/ 	
  ...
  LCD_SetFont(&Font16x24);
 	...
 	LCD_DisplayStringLine_EN_CH(LINE(0), (uint8_t* )"                 特种加工电导率测量仪");
  LCD_DisplayStringLine_EN_CH(LINE(1), (uint8_t* )"  +  实验编号  -");
  LCD_DisplayStringLine_EN_CH(LINE(3), (uint8_t* )"                                 +  加工间隙  -");  
	LCD_DisplayStringLine_EN_CH(LINE(4), (uint8_t* )"     溶液温度：");
  LCD_DisplayStringLine_EN_CH(LINE(8), (uint8_t* )"                                 +  搅拌速度  -");
  LCD_DisplayStringLine_EN_CH(LINE(9), (uint8_t* )"     溶液电导率：");
  LCD_DisplayStringLine_EN_CH(LINE(13),(uint8_t* )"                                 +  加工电压  -");
  LCD_DisplayStringLine_EN_CH(LINE(14),(uint8_t* )"     加工时间：");
  LCD_DisplayStringLine_EN_CH(LINE(18),(uint8_t* )"                                            HFUT");
  LCD_DisplayStringLine_EN_CH(LINE(19),(uint8_t* )"                                           XuPeng");
...
{
  /*外部函数读取的值*/  
  EleCndct_Value = EleConductivity_Value();
  Tempt          = ds18b20_read();
  ...
  /*使用c标准库把变量转化成字符串*/
    ...
    /*=========================================================*/
  /*1-显示当前实验编号*/
	sprintf(disExperNum,     "      No.%d",ExperNum);
	LCD_ClearLine(LINE(2));
	LCD_DisplayStringLine_EN_CH(LINE(2),(uint8_t* )disExperNum);
  
  /*=========================================================*/  
  /*4-显示加工间隙*/ 
  sprintf(disGap,"                                    %3.1f mm",MachGap);
	LCD_ClearLine(LINE(5));
	LCD_DisplayStringLine_EN_CH(LINE(5),(uint8_t* )disGap);
  
  /*=========================================================*/
  /*1-显示溶液当前温度*/
	sprintf(disTempt,     "      %3.1f ℃",Tempt);
	LCD_ClearLine(LINE(6));
	LCD_DisplayStringLine_EN_CH(LINE(6),(uint8_t* )disTempt);
  
  /*=========================================================*/
  /*2-显示溶液当前电导率*/
	sprintf(disEleCndt,   "     %5.2f ms/cm", EleCndct_Value);
	LCD_ClearLine(LINE(11));
	LCD_DisplayStringLine_EN_CH(LINE(11),(uint8_t* )disEleCndt);

  /*=========================================================*/
  /*3-显示搅拌速度*/
	sprintf(disSpeed,"                                   %4d r/min",StirSpeed);
	LCD_ClearLine(LINE(10));
	LCD_DisplayStringLine_EN_CH(LINE(10),(uint8_t* )disSpeed);


  /*  显示BUG信息*/
  sprintf(disBug,"                     %s",BugBugBug);
	LCD_ClearLine(LINE(12));
	LCD_DisplayStringLine_EN_CH(LINE(12),(uint8_t* )disBug);


  /*=========================================================*/
  /*4-显示加工电压*/ 
  sprintf(disVoltage,"                                     %3.1f V",MachVoltage);
	LCD_ClearLine(LINE(15));
	LCD_DisplayStringLine_EN_CH(LINE(15),(uint8_t* )disVoltage);

  /*=========================================================*/
  /*5-显示当前加工时间*/
	sprintf(disTime,      "     %2ldM %2ldS",Running_Time / 60, Running_Time % 60);
	LCD_ClearLine(LINE(16));
	LCD_DisplayStringLine_EN_CH(LINE(16),(uint8_t* )disTime);
  ...
  switch (DeviceStatus)
  {
    case DEVICE_RUNNING :
    {
      ...
      LCD_DisplayStringLine_EN_CH(LINE(17),(uint8_t* )"                     RUNNING");
      break;
    }
    case DEVICE_STOPPED : 
    {
      LCD_ClearLine(LINE(17));
      LCD_DisplayStringLine_EN_CH(LINE(17),(uint8_t* )"                     STOPPED");
      break;
    }
    default:break;
  }
  ...
  /*将当前信息存起来*/
  sprintf(WriteBuffer,  "No.%d\n"
                        "加工时间:  %2ld   Min          加工间隙:    %3.1f  mm\n"
                        "溶液温度: %3.1f  ℃           溶液电导率: %5.2f ms/cm\n"
                        "搅拌速度:%4d   r/min        加工电压：   %3.1f  V\n\n",
                        ExperNum, Running_Time / 60, MachGap, Tempt, EleCndct_Value, StirSpeed, MachVoltage);
